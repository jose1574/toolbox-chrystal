...
class AditionalDataEntityDetail(db.Model):
    ...
    aditional_data_entity = db.relationship('AditionalDataEntity', primaryjoin='AditionalDataEntityDetail.main_correlative == AditionalDataEntity.correlative', backref='aditional_data_entity_details')
...
class AditionalDataEntityRel(db.Model):
    ...
    aditional_data_entity = db.relationship('AditionalDataEntity', primaryjoin='AditionalDataEntityRel.main_line == AditionalDataEntity.correlative', backref='aditional_data_entity_rels')
...
class ArchingBox(db.Model):
    ...
    user = db.relationship('User', primaryjoin='ArchingBox.box_user == User.code', backref='user_user_arching_boxes')
    user1 = db.relationship('User', primaryjoin='ArchingBox.close_user == User.code', backref='user_user_arching_boxes_0')
    user2 = db.relationship('User', primaryjoin='ArchingBox.open_user == User.code', backref='user_user_arching_boxes_1')
    station1 = db.relationship('Station', primaryjoin='ArchingBox.station == Station.code', backref='arching_boxes')
...
class ArchingBoxDetail(db.Model):
    ...
    coin = db.relationship('Coin', primaryjoin='ArchingBoxDetail.coin_code == Coin.code', backref='arching_box_details')
    arching_box = db.relationship('ArchingBox', primaryjoin='ArchingBoxDetail.main_correlative == ArchingBox.correlative', backref='arching_box_details')
...
class BankAccount(db.Model):
    ...
    account_accounting1 = db.relationship('AccountAccounting', primaryjoin='BankAccount.account_accounting == AccountAccounting.code', backref='bank_accounts')
    coin1 = db.relationship('Coin', primaryjoin='BankAccount.coin == Coin.code', backref='bank_accounts')
...
class BankAccountTax(db.Model):
    ...
    account_accounting = db.relationship('AccountAccounting', primaryjoin='BankAccountTax.account_accountin_code == AccountAccounting.code', backref='bank_account_taxes')
    bank_account = db.relationship('BankAccount', primaryjoin='BankAccountTax.bank_account_code == BankAccount.code', backref='bank_account_taxes')
...
class BankConciliation(db.Model):
    ...
    transfers_received_amount = db.Column(db.Double(53), server_default=db.FetchedValue())
    ...
    transfers_received_count_transit = db.Column(db.Integer, server_default=db.FetchedValue())
    transfers_received_amount_transit = db.Column(db.Double(53), server_default=db.FetchedValue())
    transfers_made_count_transit = db.Column(db.Integer, server_default=db.FetchedValue())
    transfers_made_amount_transit = db.Column(db.Double(53), server_default=db.FetchedValue())
    credit_note_count_transit = db.Column(db.Integer, server_default=db.FetchedValue())
    credit_note_amount_transit = db.Column(db.Double(53), server_default=db.FetchedValue())
    debit_note_count_transit = db.Column(db.Integer, server_default=db.FetchedValue())
    debit_note_amount_transit = db.Column(db.Double(53), server_default=db.FetchedValue())
    ...
    movil_payment_received_count = db.Column(db.Double(53), server_default=db.FetchedValue())
    movil_payment_received_amount = db.Column(db.Double(53), server_default=db.FetchedValue())
    movil_payment_made_count = db.Column(db.Double(53), server_default=db.FetchedValue())
    movil_payment_made_amount = db.Column(db.Double(53), server_default=db.FetchedValue())

    bank_account1 = db.relationship('BankAccount', primaryjoin='BankConciliation.bank_account == BankAccount.code', backref='bank_conciliations')
...
class BankTransactionAccountDetail(db.Model):
    ...
    account_accounting1 = db.relationship('AccountAccounting', primaryjoin='BankTransactionAccountDetail.account_accounting == AccountAccounting.code', backref='bank_transaction_account_details')
    bank_transaction = db.relationship('BankTransaction', primaryjoin='BankTransactionAccountDetail.main_correlative == BankTransaction.correlative', backref='bank_transaction_account_details')
...
class BankTransaction(db.Model):
    ...
    bank_account2 = db.relationship('BankAccount', primaryjoin='BankTransaction.bank_account_related == BankAccount.code', backref='bankaccount_bank_transactions_0')
    beneficiary1 = db.relationship('Beneficiary', primaryjoin='BankTransaction.beneficiary == Beneficiary.code', backref='bank_transactions')
    station1 = db.relationship('Station', primaryjoin='BankTransaction.station == Station.code', backref='bank_transactions')
    user = db.relationship('User', primaryjoin='BankTransaction.user_code == User.code', backref='bank_transactions')
...
class Browser(db.Model):
    ...
    coin = db.relationship('Coin', primaryjoin='Browser.coin_code == Coin.code', backref='browsers')
...
class BrowserColumn(db.Model):
    ...
    browser = db.relationship('Browser', primaryjoin='BrowserColumn.main_code == Browser.code', backref='browser_columns')
...
class BrowserFilter(db.Model):
    ...
    browser = db.relationship('Browser', primaryjoin='BrowserFilter.main_code == Browser.code', backref='browser_filters')
...
class BrowserParameter(db.Model):
    ...
    browser = db.relationship('Browser', primaryjoin='BrowserParameter.main_code == Browser.code', backref='browser_parameters')
...
class CashOperation(db.Model):
    ...
    station = db.relationship('Station', primaryjoin='CashOperation.station_code == Station.code', backref='cash_operations')
    user = db.relationship('User', primaryjoin='CashOperation.user_code == User.code', backref='cash_operations')
...
class Client(db.Model):
    ...
    area_sale = db.relationship('AreaSale', primaryjoin='Client.area_sales == AreaSale.code', backref='clients')
    ...
    province1 = db.relationship('Province', primaryjoin='Client.province == Province.code', backref='clients')
    seller1 = db.relationship('Seller', primaryjoin='Client.seller == Seller.code', backref='clients')
    town1 = db.relationship('Town', primaryjoin='Client.town == Town.code', backref='clients')
...
class ClientsAddres(db.Model):
    ...
    client = db.relationship('Client', primaryjoin='ClientsAddres.client_code == Client.code', backref='clients_address')
...
class ClientsBalance(db.Model):
    ...
    client1 = db.relationship('Client', primaryjoin='ClientsBalance.client == Client.code', backref='clients_balances')
...
class ClosingSalesPoint(db.Model):
    ...
    bank_account1 = db.relationship('BankAccount', primaryjoin='ClosingSalesPoint.bank_account == BankAccount.code', backref='closing_sales_points')
    sale_point1 = db.relationship('SalePoint', primaryjoin='ClosingSalesPoint.sale_point == SalePoint.code', backref='closing_sales_points')
    station1 = db.relationship('Station', primaryjoin='ClosingSalesPoint.station == Station.code', backref='closing_sales_points')
    user = db.relationship('User', primaryjoin='ClosingSalesPoint.user_code == User.code', backref='closing_sales_points')
...
class ClosingSalesPointCard(db.Model):
    ...
    card_type1 = db.relationship('CardType', primaryjoin='ClosingSalesPointCard.card_type == CardType.code', backref='closing_sales_point_cards')
    closing_sales_point = db.relationship('ClosingSalesPoint', primaryjoin='ClosingSalesPointCard.main_correlative == ClosingSalesPoint.correlative', backref='closing_sales_point_cards')
...
class CodesIslr(db.Model):
    ...
    percenttaxablenaturalresident = db.Column(db.Double(53), server_default=db.FetchedValue())
    percenttaxablenaturalnoresident = db.Column(db.Double(53), server_default=db.FetchedValue())
    percentretentionnaturalresident = db.Column(db.Double(53), server_default=db.FetchedValue())
    percentretentionnaturalnoresident = db.Column(db.Double(53), server_default=db.FetchedValue())
    applytohigherpaymentnaturalresident = db.Column(db.Double(53), server_default=db.FetchedValue())
    applytohigherpaymentnaturalnoresident = db.Column(db.Double(53), server_default=db.FetchedValue())
    sustraendonaturalresidente = db.Column(db.Double(53), server_default=db.FetchedValue())
    codenaturalresidente = db.Column(db.String)
    codenaturalnoresidente = db.Column(db.String)
    percenttaxablejuridicaldomiciled = db.Column(db.Double(53), server_default=db.FetchedValue())
    percenttaxablejuridicalnodomiciled = db.Column(db.Double(53), server_default=db.FetchedValue())
    percentretentionjuridicaldomiciled = db.Column(db.Double(53), server_default=db.FetchedValue())
    percentretentionjuridicalnodomiciled = db.Column(db.Double(53), server_default=db.FetchedValue())
    applytohigherpaymentjuridicaldomiciled = db.Column(db.Double(53), server_default=db.FetchedValue())
    applytohigherpaymentjuridicalnodomiciled = db.Column(db.Double(53), server_default=db.FetchedValue())
...
class Coin(db.Model):
    ...
    show_in_browsers = db.Column(db.Boolean, nullable=False, server_default=db.FetchedValue())
    value_inventory = db.Column(db.Boolean, nullable=False, server_default=db.FetchedValue())
...
class Command(db.Model):
    ...
    department1 = db.relationship('Department', primaryjoin='Command.department == Department.code', backref='commands')
...
class Company(db.Model):
    ...
    email1 = db.relationship('Email', primaryjoin='Company.email == Email.account', backref='companies')
...
class CondAdditionalShare(db.Model):
    ...
    station1 = db.relationship('Station', primaryjoin='CondAdditionalShare.station == Station.code', backref='cond_additional_shares')
    user = db.relationship('User', primaryjoin='CondAdditionalShare.user_code == User.code', backref='cond_additional_shares')
...
class CondAdditionalShareCoin(db.Model):
    ...
    coin = db.relationship('Coin', primaryjoin='CondAdditionalShareCoin.coin_code == Coin.code', backref='cond_additional_share_coins')
    cond_additional_share = db.relationship('CondAdditionalShare', primaryjoin='CondAdditionalShareCoin.main_correlative == CondAdditionalShare.correlative', backref='cond_additional_share_coins')
...
class CondAdditionalShareDetail(db.Model):
    ...
    cond_additional_share = db.relationship('CondAdditionalShare', primaryjoin='CondAdditionalShareDetail.main_correlative == CondAdditionalShare.correlative', backref='cond_additional_share_details')
...
class CondClosureExpConcept(db.Model):
    ...
    cond_concept = db.relationship('CondConcept', primaryjoin='CondClosureExpConcept.concept_code == CondConcept.code', backref='cond_closure_exp_concepts')
    cond_closure_expense = db.relationship('CondClosureExpense', primaryjoin='CondClosureExpConcept.main_correlative == CondClosureExpense.correlative', backref='cond_closure_exp_concepts')
...
class CondClosureExpConceptCoin(db.Model):
    ...
    coin = db.relationship('Coin', primaryjoin='CondClosureExpConceptCoin.coin_code == Coin.code', backref='cond_closure_exp_concept_coins')
    cond_closure_exp_concept = db.relationship('CondClosureExpConcept', primaryjoin='and_(CondClosureExpConceptCoin.main_correlative == CondClosureExpConcept.main_correlative, CondClosureExpConceptCoin.main_line == CondClosureExpConcept.line)', backref='cond_closure_exp_concept_coins')
...
class CondClosureExpDetail(db.Model):
    ...
    cond_closure_expense = db.relationship('CondClosureExpense', primaryjoin='CondClosureExpDetail.main_correlative == CondClosureExpense.correlative', backref='cond_closure_exp_details')
...
class CondClosureExpense(db.Model):
    ...
    station1 = db.relationship('Station', primaryjoin='CondClosureExpense.station == Station.code', backref='cond_closure_expenses')
    user = db.relationship('User', primaryjoin='CondClosureExpense.user_code == User.code', backref='cond_closure_expenses')
...
class CondClosureExpenseCoin(db.Model):
    ...
    coin = db.relationship('Coin', primaryjoin='CondClosureExpenseCoin.coin_code == Coin.code', backref='cond_closure_expense_coins')
    cond_closure_expense = db.relationship('CondClosureExpense', primaryjoin='CondClosureExpenseCoin.main_correlative == CondClosureExpense.correlative', backref='cond_closure_expense_coins')
...
class CondConcept(db.Model):
    ...
    status1 = db.relationship('Status', primaryjoin='CondConcept.status == Status.code', backref='cond_concepts')
...
class CondReceipt(db.Model):
    ...
    cond_closure_expense = db.relationship('CondClosureExpense', primaryjoin='CondReceipt.correlative_rel == CondClosureExpense.correlative', backref='cond_receipts')
    client = db.relationship('Client', primaryjoin='CondReceipt.property_code == Client.code', backref='cond_receipts')
    station1 = db.relationship('Station', primaryjoin='CondReceipt.station == Station.code', backref='cond_receipts')
    user = db.relationship('User', primaryjoin='CondReceipt.user_code == User.code', backref='cond_receipts')
...
class CondReceiptCoin(db.Model):
    ...
    cond_receipt = db.relationship('CondReceipt', primaryjoin='CondReceiptCoin.main_correlative == CondReceipt.correlative', backref='cond_receipt_coins')
...
class CondReceiptConcept(db.Model):
    ...
    cond_receipt = db.relationship('CondReceipt', primaryjoin='CondReceiptConcept.main_correlative == CondReceipt.correlative', backref='cond_receipt_concepts')
...
class CondReceiptConceptCoin(db.Model):
    ...
    cond_receipt_coin = db.relationship('CondReceiptCoin', primaryjoin='and_(CondReceiptConceptCoin.main_correlative == CondReceiptCoin.main_correlative, CondReceiptConceptCoin.coin_code == CondReceiptCoin.coin_code)', backref='cond_receipt_concept_coins')
...
class Debtstopay(db.Model):
    ...
    indexing_correlative_origin = db.Column(db.Integer, server_default=db.FetchedValue())
    indexing_module_origin = db.Column(db.String, server_default=db.FetchedValue())
    indexing_register_factor_rel = db.Column(db.Double(53), server_default=db.FetchedValue())
    ...
    coin = db.relationship('Coin', primaryjoin='Debtstopay.coin_code == Coin.code', backref='debtstopays')
    provider = db.relationship('Provider', primaryjoin='Debtstopay.provider_code == Provider.code', backref='debtstopays')
    station1 = db.relationship('Station', primaryjoin='Debtstopay.station == Station.code', backref='debtstopays')
...
class DebtstopayCoin(db.Model):
    ...
    coin = db.relationship('Coin', primaryjoin='DebtstopayCoin.coin_code == Coin.code', backref='debtstopay_coins')
    debtstopay = db.relationship('Debtstopay', primaryjoin='DebtstopayCoin.main_correlative == Debtstopay.correlative', backref='debtstopay_coins')
...
class DebtstopayDetail(db.Model):
    ...
    relation_type = db.Column(db.String, primary_key=True, nullable=False, server_default=db.FetchedValue())

    debtstopay = db.relationship('Debtstopay', primaryjoin='DebtstopayDetail.correlative_related == Debtstopay.correlative', backref='debtstopay_debtstopay_details')
    debtstopay1 = db.relationship('Debtstopay', primaryjoin='DebtstopayDetail.main_correlative == Debtstopay.correlative', backref='debtstopay_debtstopay_details_0')
...
class DebtstopayDetailsCoin(db.Model):
    ...
    debtstopay = db.relationship('Debtstopay', primaryjoin='DebtstopayDetailsCoin.correlative_related == Debtstopay.correlative', backref='debtstopay_debtstopay_details_coins')
    debtstopay1 = db.relationship('Debtstopay', primaryjoin='DebtstopayDetailsCoin.main_correlative == Debtstopay.correlative', backref='debtstopay_debtstopay_details_coins_0')
...
class DebtstopayTax(db.Model):
    ...
    tax1 = db.relationship('Tax', primaryjoin='DebtstopayTax.taxe_code == Tax.code', backref='debtstopay_taxes')
...
class DebtstopayTaxesCoin(db.Model):
    ...
    debtstopay_tax = db.relationship('DebtstopayTax', primaryjoin='and_(DebtstopayTaxesCoin.main_correlative == DebtstopayTax.main_correlative, DebtstopayTaxesCoin.main_taxe_code == DebtstopayTax.taxe_code)', backref='debtstopay_taxes_coins')
...
class DeliveryOperation(db.Model):
    ...
    driver1 = db.relationship('Driver', primaryjoin='DeliveryOperation.driver == Driver.code', backref='delivery_operations')
    station1 = db.relationship('Station', primaryjoin='DeliveryOperation.station == Station.code', backref='delivery_operations')
    user = db.relationship('User', primaryjoin='DeliveryOperation.user_code == User.code', backref='delivery_operations')
    vehicle1 = db.relationship('Vehicle', primaryjoin='DeliveryOperation.vehicle == Vehicle.code', backref='delivery_operations')
...
class Department(db.Model):
    ...
    father_department = db.Column(db.String, nullable=False, server_default=db.FetchedValue())
...
class FiscalPrinterConfig(db.Model):
    ...
    lines_product_description = db.Column(db.Integer, nullable=False, server_default=db.FetchedValue())
...
class FiscalPrinterZDetail(db.Model):
    ...
    fiscal_printer_z = db.relationship('FiscalPrinterZ', primaryjoin='FiscalPrinterZDetail.main_correlative == FiscalPrinterZ.correlative', backref='fiscal_printer_z_details')
...
class InventoryOperation(db.Model):
    ...
    location = db.relationship('Location', primaryjoin='InventoryOperation.destination_location == Location.code', backref='location_inventory_operations')
    store1 = db.relationship('Store', primaryjoin='InventoryOperation.destination_store == Store.code', backref='store_inventory_operations')
    location1 = db.relationship('Location', primaryjoin='InventoryOperation.locations == Location.code', backref='location_inventory_operations_0')
    station1 = db.relationship('Station', primaryjoin='InventoryOperation.station == Station.code', backref='inventory_operations')
    store2 = db.relationship('Store', primaryjoin='InventoryOperation.store == Store.code', backref='store_inventory_operations_0')
    user = db.relationship('User', primaryjoin='InventoryOperation.user_code == User.code', backref='inventory_operations')
...
class InventoryOperationCoin(db.Model):
    ...
    coin = db.relationship('Coin', primaryjoin='InventoryOperationCoin.coin_code == Coin.code', backref='inventory_operation_coins')
    inventory_operation = db.relationship('InventoryOperation', primaryjoin='InventoryOperationCoin.main_correlative == InventoryOperation.correlative', backref='inventory_operation_coins')
...
class InventoryOperationDetail(db.Model):
    ...
    product = db.relationship('Product', primaryjoin='InventoryOperationDetail.code_product == Product.code', backref='inventory_operation_details')
    coin = db.relationship('Coin', primaryjoin='InventoryOperationDetail.coin_code == Coin.code', backref='inventory_operation_details')
    location = db.relationship('Location', primaryjoin='InventoryOperationDetail.destination_location == Location.code', backref='location_inventory_operation_details')
    store1 = db.relationship('Store', primaryjoin='InventoryOperationDetail.destination_store == Store.code', backref='store_inventory_operation_details')
    location1 = db.relationship('Location', primaryjoin='InventoryOperationDetail.locations == Location.code', backref='location_inventory_operation_details_0')
    inventory_operation = db.relationship('InventoryOperation', primaryjoin='InventoryOperationDetail.main_correlative == InventoryOperation.correlative', backref='inventory_operation_details')
    store2 = db.relationship('Store', primaryjoin='InventoryOperationDetail.store == Store.code', backref='store_inventory_operation_details_0')
    products_unit = db.relationship('ProductsUnit', primaryjoin='InventoryOperationDetail.unit == ProductsUnit.correlative', backref='inventory_operation_details')
...
class InventoryOperationDetailsProductsUnit(db.Model):
    ...
    products_unit = db.relationship('ProductsUnit', primaryjoin='InventoryOperationDetailsProductsUnit.correlative_units == ProductsUnit.correlative', backref='inventory_operation_details_products_units')
    inventory_operation_detail = db.relationship('InventoryOperationDetail', primaryjoin='InventoryOperationDetailsProductsUnit.main_line == InventoryOperationDetail.line', backref='inventory_operation_details_products_units')
...
class InventoryOperationDetailsSerial(db.Model):
    ...
    inventory_operation_detail = db.relationship('InventoryOperationDetail', primaryjoin='and_(InventoryOperationDetailsSerial.main_correlative == InventoryOperationDetail.main_correlative, InventoryOperationDetailsSerial.main_line == InventoryOperationDetail.line)', backref='inventory_operation_details_serials')
...
class InventoryOperationTax(db.Model):
    ...
    tax1 = db.relationship('Tax', primaryjoin='InventoryOperationTax.taxe_code == Tax.code', backref='inventory_operation_taxes')
...
class Numbering(db.Model):
    ...
    module_numbering = db.relationship('ModuleNumbering', primaryjoin='Numbering.module == ModuleNumbering.code', backref='numberings')
...
class PettyCashConciliation(db.Model):
    ...
    petty_cash1 = db.relationship('PettyCash', primaryjoin='PettyCashConciliation.petty_cash == PettyCash.code', backref='petty_cash_conciliations')
...
class PettyCashTransactionAccountDetail(db.Model):
    ...
    account_accounting1 = db.relationship('AccountAccounting', primaryjoin='PettyCashTransactionAccountDetail.account_accounting == AccountAccounting.code', backref='petty_cash_transaction_account_details')
    petty_cash_transaction = db.relationship('PettyCashTransaction', primaryjoin='PettyCashTransactionAccountDetail.main_correlative == PettyCashTransaction.correlative', backref='petty_cash_transaction_account_details')
...
class PettyCashTransaction(db.Model):
    ...
    petty_cash1 = db.relationship('PettyCash', primaryjoin='PettyCashTransaction.petty_cash == PettyCash.code', backref='petty_cash_transactions')
...
class ProdFormula(db.Model):
    ...
    product = db.relationship('Product', primaryjoin='ProdFormula.product_code == Product.code', backref='prod_formulas')
    products_unit = db.relationship('ProductsUnit', primaryjoin='ProdFormula.product_unit == ProductsUnit.correlative', backref='prod_formulas')
...
class ProdFormulasDetail(db.Model):
    ...
    formula = db.relationship('ProdFormula', primaryjoin='ProdFormulasDetail.formula_id == ProdFormula.id', backref='prod_formulas_details')
    product = db.relationship('Product', primaryjoin='ProdFormulasDetail.product_code == Product.code', backref='prod_formulas_details')
    products_unit = db.relationship('ProductsUnit', primaryjoin='ProdFormulasDetail.product_unit == ProductsUnit.correlative', backref='prod_formulas_details')
...
class Product(db.Model):
    ...
    extract_net_from_unit_cost_plus_tax = db.Column(db.Boolean, server_default=db.FetchedValue())
    extract_net_from_unit_price_plus_tax = db.Column(db.Boolean, server_default=db.FetchedValue())
    maximum_stock = db.Column(db.Double(53), server_default=db.FetchedValue())

    tax = db.relationship('Tax', primaryjoin='Product.buy_tax == Tax.code', backref='tax_products')
    coin1 = db.relationship('Coin', primaryjoin='Product.coin == Coin.code', backref='products')
    department1 = db.relationship('Department', primaryjoin='Product.department == Department.code', backref='products')
    origin1 = db.relationship('Origin', primaryjoin='Product.origin == Origin.code', backref='products')
    tax1 = db.relationship('Tax', primaryjoin='Product.sale_tax == Tax.code', backref='tax_products_0')
    status1 = db.relationship('Status', primaryjoin='Product.status == Status.code', backref='products')
    technician1 = db.relationship('Technician', primaryjoin='Product.technician == Technician.code', backref='products')
...
class ProductsCode(db.Model):
    ...
    product = db.relationship('Product', primaryjoin='ProductsCode.main_code == Product.code', backref='products_codes')
...
class ProductsCommission(db.Model):
    ...
    product = db.relationship('Product', primaryjoin='ProductsCommission.product_code == Product.code', backref='products_commissions')
...
class ProductsLot(db.Model):
    ...
    product = db.relationship('Product', primaryjoin='ProductsLot.product_code == Product.code', backref='products_lots')
...
class ProductsLotsStock(db.Model):
    ...
    location = db.relationship('Location', primaryjoin='ProductsLotsStock.locations == Location.code', backref='products_lots_stocks')
    products_lot = db.relationship('ProductsLot', primaryjoin='ProductsLotsStock.main_correlative == ProductsLot.correlative', backref='products_lots_stocks')
    store1 = db.relationship('Store', primaryjoin='ProductsLotsStock.store == Store.code', backref='products_lots_stocks')
...
class ProductsLotsUnit(db.Model):
    ...
    products_lot = db.relationship('ProductsLot', primaryjoin='ProductsLotsUnit.lots_correlative == ProductsLot.correlative', backref='products_lots_units')
    products_unit = db.relationship('ProductsUnit', primaryjoin='ProductsLotsUnit.main_correlative == ProductsUnit.correlative', backref='products_lots_units')
...
class ProductsPart(db.Model):
    ...
    product = db.relationship('Product', primaryjoin='ProductsPart.main_code == Product.code', backref='product_products_parts')
    product1 = db.relationship('Product', primaryjoin='ProductsPart.part_code == Product.code', backref='product_products_parts_0')
    products_unit = db.relationship('ProductsUnit', primaryjoin='ProductsPart.unit == ProductsUnit.correlative', backref='products_parts')
...
class ProductsProvider(db.Model):
    ...
    product = db.relationship('Product', primaryjoin='ProductsProvider.product_code == Product.code', backref='products_providers')
    provider = db.relationship('Provider', primaryjoin='ProductsProvider.provider_code == Provider.code', backref='products_providers')
...
class ProductsSerial(db.Model):
    ...
    location = db.relationship('Location', primaryjoin='ProductsSerial.locations == Location.code', backref='products_serials')
    product = db.relationship('Product', primaryjoin='ProductsSerial.product_code == Product.code', backref='products_serials')
    store1 = db.relationship('Store', primaryjoin='ProductsSerial.store == Store.code', backref='products_serials')
...
class ProductsStatistic(db.Model):
    ...
    download_amount_by_transfer = db.Column(db.Double(53), server_default=db.FetchedValue())
    load_amount_by_adjustment = db.Column(db.Double(53), server_default=db.FetchedValue())
    download_amount_by_adjustment = db.Column(db.Double(53), server_default=db.FetchedValue())
    sales_delivery_note_amount = db.Column(db.Double(53), server_default=db.FetchedValue())
    ...
    sales_credit_note_amount = db.Column(db.Double(53), server_default=db.FetchedValue())
    shopping_delivery_note_amount = db.Column(db.Double(53), server_default=db.FetchedValue())
    shopping_bill_amount = db.Column(db.Double(53), server_default=db.FetchedValue())
    shopping_devolution_amount = db.Column(db.Double(53), server_default=db.FetchedValue())
    shopping_credit_note_amount = db.Column(db.Double(53), server_default=db.FetchedValue())
    ...
    internal_download_amount = db.Column(db.Double(53), server_default=db.FetchedValue())
    shopping_expense_amount = db.Column(db.Double(53), server_default=db.FetchedValue())

    location = db.relationship('Location', primaryjoin='ProductsStatistic.locations == Location.code', backref='products_statistics')
    product = db.relationship('Product', primaryjoin='ProductsStatistic.product_code == Product.code', backref='products_statistics')
    store1 = db.relationship('Store', primaryjoin='ProductsStatistic.store == Store.code', backref='products_statistics')
...
class ProductsStock(db.Model):
    ...
    location = db.relationship('Location', primaryjoin='ProductsStock.locations == Location.code', backref='products_stocks')
    product = db.relationship('Product', primaryjoin='ProductsStock.product_code == Product.code', backref='products_stocks')
    store1 = db.relationship('Store', primaryjoin='ProductsStock.store == Store.code', backref='products_stocks')
...
class ProductsUnit(db.Model):
    ...
    product = db.relationship('Product', primaryjoin='ProductsUnit.product_code == Product.code', backref='products_units')
    unit1 = db.relationship('Unit', primaryjoin='ProductsUnit.unit == Unit.code', backref='products_units')
...
class Provider(db.Model):
    ...
    percent_municipal_retention = db.Column(db.Double(53), server_default=db.FetchedValue())
    retention_tax_agent = db.Column(db.Boolean, server_default=db.FetchedValue())
    retention_municipal_agent = db.Column(db.Boolean, server_default=db.FetchedValue())
    retention_islr_agent = db.Column(db.Boolean, server_default=db.FetchedValue())
    perception_igtf_agent = db.Column(db.Boolean, server_default=db.FetchedValue())

    city1 = db.relationship('City', primaryjoin='Provider.city == City.code', backref='providers')
    country1 = db.relationship('Country', primaryjoin='Provider.country == Country.code', backref='providers')
    person_type = db.relationship('PersonType', primaryjoin='Provider.provider_type == PersonType.code', backref='providers')
    province1 = db.relationship('Province', primaryjoin='Provider.province == Province.code', backref='providers')
    town1 = db.relationship('Town', primaryjoin='Provider.town == Town.code', backref='providers')
...
class ProvidersBalance(db.Model):
    ...
    provider1 = db.relationship('Provider', primaryjoin='ProvidersBalance.provider == Provider.code', backref='providers_balances')
...
class Receivable(db.Model):
    ...
    client = db.relationship('Client', primaryjoin='Receivable.client_code == Client.code', backref='receivables')
    coin = db.relationship('Coin', primaryjoin='Receivable.coin_code == Coin.code', backref='receivables')
    seller1 = db.relationship('Seller', primaryjoin='Receivable.seller == Seller.code', backref='receivables')
    station1 = db.relationship('Station', primaryjoin='Receivable.station == Station.code', backref='receivables')
    user = db.relationship('User', primaryjoin='Receivable.user_code == User.code', backref='receivables')
...
class ReceivableCoin(db.Model):
    ...
    coin = db.relationship('Coin', primaryjoin='ReceivableCoin.coin_code == Coin.code', backref='receivable_coins')
    receivable = db.relationship('Receivable', primaryjoin='ReceivableCoin.main_correlative == Receivable.correlative', backref='receivable_coins')
...
class ReceivableDetail(db.Model):
    ...
    receivable = db.relationship('Receivable', primaryjoin='ReceivableDetail.correlative_related == Receivable.correlative', backref='receivable_receivable_details')
    receivable1 = db.relationship('Receivable', primaryjoin='ReceivableDetail.main_correlative == Receivable.correlative', backref='receivable_receivable_details_0')
...
class ReceivableDetailsCoin(db.Model):
    ...
    receivable = db.relationship('Receivable', primaryjoin='ReceivableDetailsCoin.correlative_related == Receivable.correlative', backref='receivable_receivable_details_coins')
    receivable1 = db.relationship('Receivable', primaryjoin='ReceivableDetailsCoin.main_correlative == Receivable.correlative', backref='receivable_receivable_details_coins_0')
...
class ReceivableTax(db.Model):
    ...
    tax1 = db.relationship('Tax', primaryjoin='ReceivableTax.taxe_code == Tax.code', backref='receivable_taxes')
...
class ReceivableTaxesCoin(db.Model):
    ...
    receivable_tax = db.relationship('ReceivableTax', primaryjoin='and_(ReceivableTaxesCoin.main_correlative == ReceivableTax.main_correlative, ReceivableTaxesCoin.main_taxe_code == ReceivableTax.taxe_code)', backref='receivable_taxes_coins')
...
class RecyclerRetentionIslrClientsDetail(db.Model):
    ...
    codes_islr = db.relationship('CodesIslr', primaryjoin='RecyclerRetentionIslrClientsDetail.code_islr == CodesIslr.code', backref='recycler_retention_islr_clients_details')
    recycler_retention_islr_client = db.relationship('RecyclerRetentionIslrClient', primaryjoin='RecyclerRetentionIslrClientsDetail.main_correlative == RecyclerRetentionIslrClient.correlative', backref='recycler_retention_islr_clients_details')
...
class RecyclerRetentionIslrProviderDetail(db.Model):
    ...
    codes_islr = db.relationship('CodesIslr', primaryjoin='RecyclerRetentionIslrProviderDetail.code_islr == CodesIslr.code', backref='recycler_retention_islr_provider_details')
    recycler_retention_islr_provider = db.relationship('RecyclerRetentionIslrProvider', primaryjoin='RecyclerRetentionIslrProviderDetail.main_correlative == RecyclerRetentionIslrProvider.correlative', backref='recycler_retention_islr_provider_details')
...
class RecyclerRetentionMunicipalClientsDetail(db.Model):
    ...
    recycler_retention_municipal_client = db.relationship('RecyclerRetentionMunicipalClient', primaryjoin='RecyclerRetentionMunicipalClientsDetail.main_correlative == RecyclerRetentionMunicipalClient.correlative', backref='recycler_retention_municipal_clients_details')
...
class RecyclerRetentionMunicipalProviderDetail(db.Model):
    ...
    recycler_retention_municipal_provider = db.relationship('RecyclerRetentionMunicipalProvider', primaryjoin='RecyclerRetentionMunicipalProviderDetail.main_correlative == RecyclerRetentionMunicipalProvider.correlative', backref='recycler_retention_municipal_provider_details')
...
class RecyclerRetentionTaxClientsDetail(db.Model):
    ...
    retention_tax_client = db.relationship('RetentionTaxClient', primaryjoin='RecyclerRetentionTaxClientsDetail.main_correlative == RetentionTaxClient.correlative', backref='recycler_retention_tax_clients_details')
...
class RecyclerRetentionTaxProviderDetail(db.Model):
    ...
    recycler_retention_tax_provider = db.relationship('RecyclerRetentionTaxProvider', primaryjoin='RecyclerRetentionTaxProviderDetail.main_correlative == RecyclerRetentionTaxProvider.correlative', backref='recycler_retention_tax_provider_details')
...
class RecyclerSalesDocumentsRel(db.Model):
    ...
    recycler_sales_operation = db.relationship('RecyclerSalesOperation', primaryjoin='RecyclerSalesDocumentsRel.main_correlative == RecyclerSalesOperation.correlative', backref='recycler_sales_documents_rels')
...
class RecyclerSalesOperation(db.Model):
    ...
    client = db.relationship('Client', primaryjoin='RecyclerSalesOperation.client_code == Client.code', backref='recycler_sales_operations')
    coin = db.relationship('Coin', primaryjoin='RecyclerSalesOperation.coin_code == Coin.code', backref='recycler_sales_operations')
    tax = db.relationship('Tax', primaryjoin='RecyclerSalesOperation.freight_tax == Tax.code', backref='recycler_sales_operations')
    location = db.relationship('Location', primaryjoin='RecyclerSalesOperation.locations == Location.code', backref='recycler_sales_operations')
    seller1 = db.relationship('Seller', primaryjoin='RecyclerSalesOperation.seller == Seller.code', backref='recycler_sales_operations')
    station1 = db.relationship('Station', primaryjoin='RecyclerSalesOperation.station == Station.code', backref='recycler_sales_operations')
    store1 = db.relationship('Store', primaryjoin='RecyclerSalesOperation.store == Store.code', backref='recycler_sales_operations')
    user = db.relationship('User', primaryjoin='RecyclerSalesOperation.user_code == User.code', backref='recycler_sales_operations')
...
class RecyclerSalesOperationCoin(db.Model):
    ...
    buy_aliquot = db.Column(db.Double(53))
    sales_aliquot = db.Column(db.Double(53))
    total_net_details = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())
    total_tax_details = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())
    total_details = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())
    discount = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())
    freight = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())
    total_net = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())
    total_tax = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())
    ...
    total_net_cost = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())
    total_tax_cost = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())
    total_cost = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())
    total_operation = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())
    total_retention_tax = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())
    total_retention_municipal = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())
    total_retention_islr = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())
    retention_tax_prorration = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())
    retention_islr_prorration = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())
    retention_municipal_prorration = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())

    coin = db.relationship('Coin', primaryjoin='RecyclerSalesOperationCoin.coin_code == Coin.code', backref='recycler_sales_operation_coins')
    recycler_sales_operation = db.relationship('RecyclerSalesOperation', primaryjoin='RecyclerSalesOperationCoin.main_correlative == RecyclerSalesOperation.correlative', backref='recycler_sales_operation_coins')
...
class RecyclerSalesOperationDetail(db.Model):
    ...
    coin = db.relationship('Coin', primaryjoin='RecyclerSalesOperationDetail.coin_code == Coin.code', backref='recycler_sales_operation_details')
    location = db.relationship('Location', primaryjoin='RecyclerSalesOperationDetail.locations == Location.code', backref='recycler_sales_operation_details')
    recycler_sales_operation = db.relationship('RecyclerSalesOperation', primaryjoin='RecyclerSalesOperationDetail.main_correlative == RecyclerSalesOperation.correlative', backref='recycler_sales_operation_details')
    tax = db.relationship('Tax', primaryjoin='RecyclerSalesOperationDetail.sale_tax == Tax.code', backref='recycler_sales_operation_details')
    store1 = db.relationship('Store', primaryjoin='RecyclerSalesOperationDetail.store == Store.code', backref='recycler_sales_operation_details')
    technician1 = db.relationship('Technician', primaryjoin='RecyclerSalesOperationDetail.technician == Technician.code', backref='recycler_sales_operation_details')
    products_unit = db.relationship('ProductsUnit', primaryjoin='RecyclerSalesOperationDetail.unit == ProductsUnit.correlative', backref='recycler_sales_operation_details')
...
class RecyclerSalesOperationDetailsLoad(db.Model):
    ...
    recycler_sales_operation_detail = db.relationship('RecyclerSalesOperationDetail', primaryjoin='and_(RecyclerSalesOperationDetailsLoad.load_line == RecyclerSalesOperationDetail.line, RecyclerSalesOperationDetailsLoad.load_correlative == RecyclerSalesOperationDetail.main_correlative)', backref='recyclersalesoperationdetail_recycler_sales_operation_details_loads')
    recycler_sales_operation_detail1 = db.relationship('RecyclerSalesOperationDetail', primaryjoin='and_(RecyclerSalesOperationDetailsLoad.main_line == RecyclerSalesOperationDetail.line, RecyclerSalesOperationDetailsLoad.main_correlative == RecyclerSalesOperationDetail.main_correlative)', backref='recyclersalesoperationdetail_recycler_sales_operation_details_loads_0')
...
class RecyclerSalesOperationDetailsPart(db.Model):
    ...
    location = db.relationship('Location', primaryjoin='RecyclerSalesOperationDetailsPart.locations == Location.code', backref='recycler_sales_operation_details_parts')
    recycler_sales_operation_detail = db.relationship('RecyclerSalesOperationDetail', primaryjoin='and_(RecyclerSalesOperationDetailsPart.main_correlative == RecyclerSalesOperationDetail.main_correlative, RecyclerSalesOperationDetailsPart.main_line == RecyclerSalesOperationDetail.line)', backref='recycler_sales_operation_details_parts')
    tax = db.relationship('Tax', primaryjoin='RecyclerSalesOperationDetailsPart.sale_tax == Tax.code', backref='recycler_sales_operation_details_parts')
    store1 = db.relationship('Store', primaryjoin='RecyclerSalesOperationDetailsPart.store == Store.code', backref='recycler_sales_operation_details_parts')
    technician1 = db.relationship('Technician', primaryjoin='RecyclerSalesOperationDetailsPart.technician == Technician.code', backref='recycler_sales_operation_details_parts')
    products_unit = db.relationship('ProductsUnit', primaryjoin='RecyclerSalesOperationDetailsPart.unit == ProductsUnit.correlative', backref='recycler_sales_operation_details_parts')
...
class RecyclerSalesOperationDetailsSerial(db.Model):
    ...
    recycler_sales_operation_detail = db.relationship('RecyclerSalesOperationDetail', primaryjoin='and_(RecyclerSalesOperationDetailsSerial.main_correlative == RecyclerSalesOperationDetail.main_correlative, RecyclerSalesOperationDetailsSerial.main_line == RecyclerSalesOperationDetail.line)', backref='recycler_sales_operation_details_serials')
...
class RecyclerSalesOperationTax(db.Model):
    ...
    tax1 = db.relationship('Tax', primaryjoin='RecyclerSalesOperationTax.taxe_code == Tax.code', backref='recycler_sales_operation_taxes')
...
class RecyclerSalesOperationTaxesCoin(db.Model):
    ...
    recycler_sales_operation_tax = db.relationship('RecyclerSalesOperationTax', primaryjoin='and_(RecyclerSalesOperationTaxesCoin.main_correlative == RecyclerSalesOperationTax.main_correlative, RecyclerSalesOperationTaxesCoin.main_taxe_code == RecyclerSalesOperationTax.taxe_code)', backref='recycler_sales_operation_taxes_coins')
...
class RecyclerShoppingDocumentsRel(db.Model):
    ...
    recycler_shopping_operation = db.relationship('RecyclerShoppingOperation', primaryjoin='RecyclerShoppingDocumentsRel.main_correlative == RecyclerShoppingOperation.correlative', backref='recycler_shopping_documents_rels')
...
class RecyclerShoppingOperation(db.Model):
    ...
    location = db.relationship('Location', primaryjoin='RecyclerShoppingOperation.locations == Location.code', backref='recycler_shopping_operations')
    provider = db.relationship('Provider', primaryjoin='RecyclerShoppingOperation.provider_code == Provider.code', backref='recycler_shopping_operations')
    station1 = db.relationship('Station', primaryjoin='RecyclerShoppingOperation.station == Station.code', backref='recycler_shopping_operations')
    store1 = db.relationship('Store', primaryjoin='RecyclerShoppingOperation.store == Store.code', backref='recycler_shopping_operations')
    user = db.relationship('User', primaryjoin='RecyclerShoppingOperation.user_code == User.code', backref='recycler_shopping_operations')
...
class RecyclerShoppingOperationCoin(db.Model):
    ...
    coin = db.relationship('Coin', primaryjoin='RecyclerShoppingOperationCoin.coin_code == Coin.code', backref='recycler_shopping_operation_coins')
    recycler_shopping_operation = db.relationship('RecyclerShoppingOperation', primaryjoin='RecyclerShoppingOperationCoin.main_correlative == RecyclerShoppingOperation.correlative', backref='recycler_shopping_operation_coins')
...
class RecyclerShoppingOperationDetail(db.Model):
    ...
    product = db.relationship('Product', primaryjoin='RecyclerShoppingOperationDetail.code_product == Product.code', backref='recycler_shopping_operation_details')
    coin = db.relationship('Coin', primaryjoin='RecyclerShoppingOperationDetail.coin_code == Coin.code', backref='recycler_shopping_operation_details')
    location = db.relationship('Location', primaryjoin='RecyclerShoppingOperationDetail.locations == Location.code', backref='recycler_shopping_operation_details')
    recycler_shopping_operation = db.relationship('RecyclerShoppingOperation', primaryjoin='RecyclerShoppingOperationDetail.main_correlative == RecyclerShoppingOperation.correlative', backref='recycler_shopping_operation_details')
    store1 = db.relationship('Store', primaryjoin='RecyclerShoppingOperationDetail.store == Store.code', backref='recycler_shopping_operation_details')
    products_unit = db.relationship('ProductsUnit', primaryjoin='RecyclerShoppingOperationDetail.unit == ProductsUnit.correlative', backref='recycler_shopping_operation_details')
...
class RecyclerShoppingOperationDetailsLoad(db.Model):
    ...
    main_correlative = db.Column(db.Integer)
    load_correlative = db.Column(db.Integer)
    load_amount = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())

    recycler_shopping_operation_detail = db.relationship('RecyclerShoppingOperationDetail', primaryjoin='and_(RecyclerShoppingOperationDetailsLoad.load_correlative == RecyclerShoppingOperationDetail.main_correlative, RecyclerShoppingOperationDetailsLoad.load_line == RecyclerShoppingOperationDetail.line)', backref='recyclershoppingoperationdetail_recycler_shopping_operation_details_loads')
    recycler_shopping_operation_detail1 = db.relationship('RecyclerShoppingOperationDetail', primaryjoin='and_(RecyclerShoppingOperationDetailsLoad.main_correlative == RecyclerShoppingOperationDetail.main_correlative, RecyclerShoppingOperationDetailsLoad.main_line == RecyclerShoppingOperationDetail.line)', backref='recyclershoppingoperationdetail_recycler_shopping_operation_details_loads_0')
...
class RecyclerShoppingOperationDetailsProductsUnit(db.Model):
    ...
    products_unit = db.relationship('ProductsUnit', primaryjoin='RecyclerShoppingOperationDetailsProductsUnit.correlative_units == ProductsUnit.correlative', backref='recycler_shopping_operation_details_products_units')
    recycler_shopping_operation_detail = db.relationship('RecyclerShoppingOperationDetail', primaryjoin='RecyclerShoppingOperationDetailsProductsUnit.main_line == RecyclerShoppingOperationDetail.line', backref='recycler_shopping_operation_details_products_units')
...
class RecyclerShoppingOperationDetailsSerial(db.Model):
    ...
    recycler_shopping_operation_detail = db.relationship('RecyclerShoppingOperationDetail', primaryjoin='and_(RecyclerShoppingOperationDetailsSerial.main_correlative == RecyclerShoppingOperationDetail.main_correlative, RecyclerShoppingOperationDetailsSerial.main_line == RecyclerShoppingOperationDetail.line)', backref='recycler_shopping_operation_details_serials')
...
class RecyclerShoppingOperationTax(db.Model):
    ...
    tax1 = db.relationship('Tax', primaryjoin='RecyclerShoppingOperationTax.taxe_code == Tax.code', backref='recycler_shopping_operation_taxes')
...
class RecyclerShoppingOperationTaxesCoin(db.Model):
    ...
    recycler_shopping_operation_tax = db.relationship('RecyclerShoppingOperationTax', primaryjoin='and_(RecyclerShoppingOperationTaxesCoin.main_correlative == RecyclerShoppingOperationTax.main_correlative, RecyclerShoppingOperationTaxesCoin.main_taxe_code == RecyclerShoppingOperationTax.taxe_code)', backref='recycler_shopping_operation_taxes_coins')
...
class RecyclerWayToPay(db.Model):
    ...
    coin = db.relationship('Coin', primaryjoin='RecyclerWayToPay.coin_code == Coin.code', backref='recycler_way_to_pays')
    station1 = db.relationship('Station', primaryjoin='RecyclerWayToPay.station == Station.code', backref='recycler_way_to_pays')
    user = db.relationship('User', primaryjoin='RecyclerWayToPay.user_code == User.code', backref='recycler_way_to_pays')
...
class RecyclerWayToPayCoin(db.Model):
    ...
    coin = db.relationship('Coin', primaryjoin='RecyclerWayToPayCoin.coin_code == Coin.code', backref='recycler_way_to_pay_coins')
    recycler_way_to_pay = db.relationship('RecyclerWayToPay', primaryjoin='RecyclerWayToPayCoin.main_correlative == RecyclerWayToPay.correlative', backref='recycler_way_to_pay_coins')
...
class RecyclerWayToPayDetail(db.Model):
    ...
    card_type1 = db.relationship('CardType', primaryjoin='RecyclerWayToPayDetail.card_type == CardType.code', backref='recycler_way_to_pay_details')
    coin = db.relationship('Coin', primaryjoin='RecyclerWayToPayDetail.coin_code == Coin.code', backref='recycler_way_to_pay_details')
    recycler_way_to_pay = db.relationship('RecyclerWayToPay', primaryjoin='RecyclerWayToPayDetail.main_correlative == RecyclerWayToPay.correlative', backref='recycler_way_to_pay_details')
    sale_point1 = db.relationship('SalePoint', primaryjoin='RecyclerWayToPayDetail.sale_point == SalePoint.code', backref='recycler_way_to_pay_details')
...
class RecyclerWayToPayDetailsDet(db.Model):
    ...
    recycler_way_to_pay_detail = db.relationship('RecyclerWayToPayDetail', primaryjoin='and_(RecyclerWayToPayDetailsDet.main_correlative == RecyclerWayToPayDetail.main_correlative, RecyclerWayToPayDetailsDet.main_line == RecyclerWayToPayDetail.line)', backref='recycler_way_to_pay_details_dets')
...
class Report(db.Model):
    ...
    menu1 = db.relationship('Menu', primaryjoin='Report.menu == Menu.code', backref='reports')
...
class ReportChart(db.Model):
    ...
    report_query = db.relationship('ReportQuery', primaryjoin='and_(ReportChart.main_code == ReportQuery.main_code_display_format, ReportChart.index_order_display_format == ReportQuery.index_order_display_format, ReportChart.index_order_query == ReportQuery.index_order)', backref='report_charts')
...
class ReportChartsCategory(db.Model):
    ...
    report_chart = db.relationship('ReportChart', primaryjoin='ReportChartsCategory.main_correlative == ReportChart.correlative', backref='report_charts_categories')
...
class ReportChartsSery(db.Model):
    ...
    report_chart = db.relationship('ReportChart', primaryjoin='ReportChartsSery.main_correlative == ReportChart.correlative', backref='report_charts_series')
...
class ReportDisplayFormat(db.Model):
    ...
    report = db.relationship('Report', primaryjoin='ReportDisplayFormat.main_code == Report.code', backref='report_display_formats')
...
class ReportField(db.Model):
    ...
    report_query = db.relationship('ReportQuery', primaryjoin='and_(ReportField.main_code_display_format == ReportQuery.main_code_display_format, ReportField.index_order_display_format == ReportQuery.index_order_display_format, ReportField.index_order_query == ReportQuery.index_order)', backref='report_fields')
...
class ReportFilter(db.Model):
    ...
    report_panel = db.relationship('ReportPanel', primaryjoin='and_(ReportFilter.main_code == ReportPanel.main_code, ReportFilter.main_panel == ReportPanel.index_order)', backref='report_filters')
...
class ReportPanel(db.Model):
    ...
    report = db.relationship('Report', primaryjoin='ReportPanel.main_code == Report.code', backref='report_panels')
...
class ReportParameter(db.Model):
    ...
    report = db.relationship('Report', primaryjoin='ReportParameter.main_code == Report.code', backref='report_parameters')
...
class ReportQuery(db.Model):
    ...
    report_display_format = db.relationship('ReportDisplayFormat', primaryjoin='and_(ReportQuery.main_code_display_format == ReportDisplayFormat.main_code, ReportQuery.index_order_display_format == ReportDisplayFormat.index_order)', backref='report_queries')
...
class RetentionIslrClientsDetail(db.Model):
    ...
    codes_islr = db.relationship('CodesIslr', primaryjoin='RetentionIslrClientsDetail.code_islr == CodesIslr.code', backref='retention_islr_clients_details')
    retention_islr_client = db.relationship('RetentionIslrClient', primaryjoin='RetentionIslrClientsDetail.main_correlative == RetentionIslrClient.correlative', backref='retention_islr_clients_details')
...
class RetentionIslrProviderDetail(db.Model):
    ...
    codes_islr = db.relationship('CodesIslr', primaryjoin='RetentionIslrProviderDetail.code_islr == CodesIslr.code', backref='retention_islr_provider_details')
    retention_islr_provider = db.relationship('RetentionIslrProvider', primaryjoin='RetentionIslrProviderDetail.main_correlative == RetentionIslrProvider.correlative', backref='retention_islr_provider_details')
...
class RetentionMunicipalClientsDetail(db.Model):
    ...
    retention_municipal_client = db.relationship('RetentionMunicipalClient', primaryjoin='RetentionMunicipalClientsDetail.main_correlative == RetentionMunicipalClient.correlative', backref='retention_municipal_clients_details')
...
class RetentionMunicipalProviderDetail(db.Model):
    ...
    retention_municipal_provider = db.relationship('RetentionMunicipalProvider', primaryjoin='RetentionMunicipalProviderDetail.main_correlative == RetentionMunicipalProvider.correlative', backref='retention_municipal_provider_details')
...
class RetentionTaxClientsDetail(db.Model):
    ...
    retention_tax_client = db.relationship('RetentionTaxClient', primaryjoin='RetentionTaxClientsDetail.main_correlative == RetentionTaxClient.correlative', backref='retention_tax_clients_details')
...
class RetentionTaxProviderDetail(db.Model):
    ...
    retention_tax_provider = db.relationship('RetentionTaxProvider', primaryjoin='RetentionTaxProviderDetail.main_correlative == RetentionTaxProvider.correlative', backref='retention_tax_provider_details')
...
class SalePointsDetail(db.Model):
    ...
    card_type = db.relationship('CardType', primaryjoin='SalePointsDetail.card_type_code == CardType.code', backref='sale_points_details')
    sale_point = db.relationship('SalePoint', primaryjoin='SalePointsDetail.sale_point_code == SalePoint.code', backref='sale_points_details')
...
class SalesDocumentsRel(db.Model):
    ...
    sales_operation = db.relationship('SalesOperation', primaryjoin='SalesDocumentsRel.main_correlative == SalesOperation.correlative', backref='sales_documents_rels')
...
class SalesOperation(db.Model):
    ...
    client = db.relationship('Client', primaryjoin='SalesOperation.client_code == Client.code', backref='sales_operations')
    coin = db.relationship('Coin', primaryjoin='SalesOperation.coin_code == Coin.code', backref='sales_operations')
    tax = db.relationship('Tax', primaryjoin='SalesOperation.freight_tax == Tax.code', backref='sales_operations')
    location = db.relationship('Location', primaryjoin='SalesOperation.locations == Location.code', backref='sales_operations')
    seller1 = db.relationship('Seller', primaryjoin='SalesOperation.seller == Seller.code', backref='sales_operations')
    station1 = db.relationship('Station', primaryjoin='SalesOperation.station == Station.code', backref='sales_operations')
    store1 = db.relationship('Store', primaryjoin='SalesOperation.store == Store.code', backref='sales_operations')
    user = db.relationship('User', primaryjoin='SalesOperation.user_code == User.code', backref='sales_operations')
...
class SalesOperationCoin(db.Model):
    ...
    coin = db.relationship('Coin', primaryjoin='SalesOperationCoin.coin_code == Coin.code', backref='sales_operation_coins')
    sales_operation = db.relationship('SalesOperation', primaryjoin='SalesOperationCoin.main_correlative == SalesOperation.correlative', backref='sales_operation_coins')
...
class SalesOperationDetail(db.Model):
    ...
    coin = db.relationship('Coin', primaryjoin='SalesOperationDetail.coin_code == Coin.code', backref='sales_operation_details')
    location = db.relationship('Location', primaryjoin='SalesOperationDetail.locations == Location.code', backref='sales_operation_details')
    sales_operation = db.relationship('SalesOperation', primaryjoin='SalesOperationDetail.main_correlative == SalesOperation.correlative', backref='sales_operation_details')
    tax = db.relationship('Tax', primaryjoin='SalesOperationDetail.sale_tax == Tax.code', backref='sales_operation_details')
    store1 = db.relationship('Store', primaryjoin='SalesOperationDetail.store == Store.code', backref='sales_operation_details')
    technician1 = db.relationship('Technician', primaryjoin='SalesOperationDetail.technician == Technician.code', backref='sales_operation_details')
    products_unit = db.relationship('ProductsUnit', primaryjoin='SalesOperationDetail.unit == ProductsUnit.correlative', backref='sales_operation_details')
...
class SalesOperationDetailsLoad(db.Model):
    ...
    sales_operation_detail = db.relationship('SalesOperationDetail', primaryjoin='and_(SalesOperationDetailsLoad.load_line == SalesOperationDetail.line, SalesOperationDetailsLoad.load_correlative == SalesOperationDetail.main_correlative)', backref='salesoperationdetail_sales_operation_details_loads')
    sales_operation_detail1 = db.relationship('SalesOperationDetail', primaryjoin='and_(SalesOperationDetailsLoad.main_line == SalesOperationDetail.line, SalesOperationDetailsLoad.main_correlative == SalesOperationDetail.main_correlative)', backref='salesoperationdetail_sales_operation_details_loads_0')
...
class SalesOperationDetailsPart(db.Model):
    ...
    location = db.relationship('Location', primaryjoin='SalesOperationDetailsPart.locations == Location.code', backref='sales_operation_details_parts')
    sales_operation_detail = db.relationship('SalesOperationDetail', primaryjoin='and_(SalesOperationDetailsPart.main_correlative == SalesOperationDetail.main_correlative, SalesOperationDetailsPart.main_line == SalesOperationDetail.line)', backref='sales_operation_details_parts')
    tax = db.relationship('Tax', primaryjoin='SalesOperationDetailsPart.sale_tax == Tax.code', backref='sales_operation_details_parts')
    store1 = db.relationship('Store', primaryjoin='SalesOperationDetailsPart.store == Store.code', backref='sales_operation_details_parts')
    technician1 = db.relationship('Technician', primaryjoin='SalesOperationDetailsPart.technician == Technician.code', backref='sales_operation_details_parts')
    products_unit = db.relationship('ProductsUnit', primaryjoin='SalesOperationDetailsPart.unit == ProductsUnit.correlative', backref='sales_operation_details_parts')
...
class SalesOperationDetailsSerial(db.Model):
    ...
    sales_operation_detail = db.relationship('SalesOperationDetail', primaryjoin='and_(SalesOperationDetailsSerial.main_correlative == SalesOperationDetail.main_correlative, SalesOperationDetailsSerial.main_line == SalesOperationDetail.line)', backref='sales_operation_details_serials')
...
class SalesOperationTax(db.Model):
    ...
    tax1 = db.relationship('Tax', primaryjoin='SalesOperationTax.taxe_code == Tax.code', backref='sales_operation_taxes')
...
class SalesOperationTaxesCoin(db.Model):
    ...
    sales_operation_tax = db.relationship('SalesOperationTax', primaryjoin='and_(SalesOperationTaxesCoin.main_correlative == SalesOperationTax.main_correlative, SalesOperationTaxesCoin.main_taxe_code == SalesOperationTax.taxe_code)', backref='sales_operation_taxes_coins')
...
class Seller(db.Model):
    ...
    user_code = db.Column(db.String)
    percent_gerencial_debit_note = db.Column(db.Double(53), server_default=db.FetchedValue())
    percent_gerencial_credit_note = db.Column(db.Double(53), server_default=db.FetchedValue())
    percent_returned_check = db.Column(db.Double(53), server_default=db.FetchedValue())

    status1 = db.relationship('Status', primaryjoin='Seller.status == Status.code', backref='sellers')
...
class SellersRangeCommission(db.Model):
    ...
    seller = db.relationship('Seller', primaryjoin='SellersRangeCommission.seller_code == Seller.code', backref='sellers_range_commissions')
...
class ShoppingDocumentsRel(db.Model):
    ...
    shopping_operation = db.relationship('ShoppingOperation', primaryjoin='ShoppingDocumentsRel.main_correlative == ShoppingOperation.correlative', backref='shopping_documents_rels')
...
class ShoppingOperation(db.Model):
    ...
    coin = db.relationship('Coin', primaryjoin='ShoppingOperation.coin_code == Coin.code', backref='coin_shopping_operations')
    location = db.relationship('Location', primaryjoin='ShoppingOperation.locations == Location.code', backref='shopping_operations')
    provider = db.relationship('Provider', primaryjoin='ShoppingOperation.provider_code == Provider.code', backref='shopping_operations')
    coin1 = db.relationship('Coin', primaryjoin='ShoppingOperation.secondary_coin == Coin.code', backref='coin_shopping_operations_0')
    station1 = db.relationship('Station', primaryjoin='ShoppingOperation.station == Station.code', backref='shopping_operations')
    store1 = db.relationship('Store', primaryjoin='ShoppingOperation.store == Store.code', backref='shopping_operations')
    user = db.relationship('User', primaryjoin='ShoppingOperation.user_code == User.code', backref='shopping_operations')
    cond_receipt = db.relationship('CondReceipt', secondary='cond_receipt_details', backref='shopping_operations')
...
class CondShoppingOperation(ShoppingOperation):
    ...
    client = db.relationship('Client', primaryjoin='CondShoppingOperation.property_code == Client.code', backref='cond_shopping_operations')
...
class ShoppingOperationCoin(db.Model):
    ...
    coin = db.relationship('Coin', primaryjoin='ShoppingOperationCoin.coin_code == Coin.code', backref='shopping_operation_coins')
    shopping_operation = db.relationship('ShoppingOperation', primaryjoin='ShoppingOperationCoin.main_correlative == ShoppingOperation.correlative', backref='shopping_operation_coins')
...
class ShoppingOperationDetail(db.Model):
    ...
    product = db.relationship('Product', primaryjoin='ShoppingOperationDetail.code_product == Product.code', backref='shopping_operation_details')
    coin = db.relationship('Coin', primaryjoin='ShoppingOperationDetail.coin_code == Coin.code', backref='shopping_operation_details')
    location = db.relationship('Location', primaryjoin='ShoppingOperationDetail.locations == Location.code', backref='shopping_operation_details')
    shopping_operation = db.relationship('ShoppingOperation', primaryjoin='ShoppingOperationDetail.main_correlative == ShoppingOperation.correlative', backref='shopping_operation_details')
    store1 = db.relationship('Store', primaryjoin='ShoppingOperationDetail.store == Store.code', backref='shopping_operation_details')
    products_unit = db.relationship('ProductsUnit', primaryjoin='ShoppingOperationDetail.unit == ProductsUnit.correlative', backref='shopping_operation_details')
...
class ShoppingOperationDetailsLoad(db.Model):
    ...
    shopping_operation_detail = db.relationship('ShoppingOperationDetail', primaryjoin='and_(ShoppingOperationDetailsLoad.load_correlative == ShoppingOperationDetail.main_correlative, ShoppingOperationDetailsLoad.load_line == ShoppingOperationDetail.line)', backref='shoppingoperationdetail_shopping_operation_details_loads')
    shopping_operation_detail1 = db.relationship('ShoppingOperationDetail', primaryjoin='and_(ShoppingOperationDetailsLoad.main_correlative == ShoppingOperationDetail.main_correlative, ShoppingOperationDetailsLoad.main_line == ShoppingOperationDetail.line)', backref='shoppingoperationdetail_shopping_operation_details_loads_0')
...
class ShoppingOperationDetailsProductsUnit(db.Model):
    ...
    products_unit = db.relationship('ProductsUnit', primaryjoin='ShoppingOperationDetailsProductsUnit.correlative_units == ProductsUnit.correlative', backref='shopping_operation_details_products_units')
    shopping_operation_detail = db.relationship('ShoppingOperationDetail', primaryjoin='ShoppingOperationDetailsProductsUnit.main_line == ShoppingOperationDetail.line', backref='shopping_operation_details_products_units')
...
class ShoppingOperationDetailsSerial(db.Model):
    ...
    shopping_operation_detail = db.relationship('ShoppingOperationDetail', primaryjoin='and_(ShoppingOperationDetailsSerial.main_correlative == ShoppingOperationDetail.main_correlative, ShoppingOperationDetailsSerial.main_line == ShoppingOperationDetail.line)', backref='shopping_operation_details_serials')
...
class ShoppingOperationTax(db.Model):
    ...
    tax1 = db.relationship('Tax', primaryjoin='ShoppingOperationTax.taxe_code == Tax.code', backref='shopping_operation_taxes')
...
class ShoppingOperationTaxesCoin(db.Model):
    ...
    shopping_operation_tax = db.relationship('ShoppingOperationTax', primaryjoin='and_(ShoppingOperationTaxesCoin.main_correlative == ShoppingOperationTax.main_correlative, ShoppingOperationTaxesCoin.main_taxe_code == ShoppingOperationTax.taxe_code)', backref='shopping_operation_taxes_coins')
...
class Station(db.Model):
    ...
    numeration_sales_point_bill = db.Column(db.String, nullable=False, server_default=db.FetchedValue())
    ...
    sale_point2 = db.relationship('SalePoint', primaryjoin='Station.sale_point == SalePoint.code', backref='salepoint_stations_0')
...
class StationsCommand(db.Model):
    ...
    command1 = db.relationship('Command', primaryjoin='StationsCommand.command == Command.code', backref='stations_commands')
    station1 = db.relationship('Station', primaryjoin='StationsCommand.station == Station.code', backref='stations_commands')
...
class SystemProperty(db.Model):
    ...
    profile1 = db.relationship('Profile', primaryjoin='SystemProperty.profile == Profile.code', backref='system_properties')
    properties_group1 = db.relationship('PropertiesGroup', primaryjoin='SystemProperty.properties_group == PropertiesGroup.code', backref='system_properties')
...
class Technician(db.Model):
    ...
    percent_commission_minimum_price = db.Column(db.Double(53), server_default=db.FetchedValue())
    percent_commission_variable_price = db.Column(db.Double(53), server_default=db.FetchedValue())

    status1 = db.relationship('Status', primaryjoin='Technician.status == Status.code', backref='technicians')
...
class User(db.Model):
    ...
    email1 = db.relationship('Email', primaryjoin='User.company_email == Email.account', backref='users')
    profile1 = db.relationship('Profile', primaryjoin='User.profile == Profile.code', backref='users')
    status1 = db.relationship('Status', primaryjoin='User.status == Status.code', backref='users')
...
class WayToPay(db.Model):
    ...
    coin = db.relationship('Coin', primaryjoin='WayToPay.coin_code == Coin.code', backref='way_to_pays')
    station1 = db.relationship('Station', primaryjoin='WayToPay.station == Station.code', backref='way_to_pays')
    user = db.relationship('User', primaryjoin='WayToPay.user_code == User.code', backref='way_to_pays')
...
class WayToPayCoin(db.Model):
    ...
    coin = db.relationship('Coin', primaryjoin='WayToPayCoin.coin_code == Coin.code', backref='way_to_pay_coins')
    way_to_pay = db.relationship('WayToPay', primaryjoin='WayToPayCoin.main_correlative == WayToPay.correlative', backref='way_to_pay_coins')
...
class WayToPayDetail(db.Model):
    ...
    bank1 = db.relationship('Bank', primaryjoin='WayToPayDetail.bank == Bank.code', backref='way_to_pay_details')
    card_type1 = db.relationship('CardType', primaryjoin='WayToPayDetail.card_type == CardType.code', backref='way_to_pay_details')
    coin = db.relationship('Coin', primaryjoin='WayToPayDetail.coin_code == Coin.code', backref='way_to_pay_details')
    way_to_pay = db.relationship('WayToPay', primaryjoin='WayToPayDetail.main_correlative == WayToPay.correlative', backref='way_to_pay_details')
    sale_point1 = db.relationship('SalePoint', primaryjoin='WayToPayDetail.sale_point == SalePoint.code', backref='way_to_pay_details')
    closing_sales_point = db.relationship('ClosingSalesPoint', secondary='closing_sales_point_way_to_pay', backref='way_to_pay_details')
...
class WayToPayDetailsDet(db.Model):
    ...
    way_to_pay_detail = db.relationship('WayToPayDetail', primaryjoin='and_(WayToPayDetailsDet.main_correlative == WayToPayDetail.main_correlative, WayToPayDetailsDet.main_line == WayToPayDetail.line)', backref='way_to_pay_details_dets')
...
class Workshop(db.Model):
    ...
    station1 = db.relationship('Station', primaryjoin='Workshop.station == Station.code', backref='workshops')
    workshop_statu = db.relationship('WorkshopStatu', primaryjoin='Workshop.status == WorkshopStatu.code', backref='workshops')
    technician1 = db.relationship('Technician', primaryjoin='Workshop.technician == Technician.code', backref='workshops')
    user = db.relationship('User', primaryjoin='Workshop.user_code == User.code', backref='workshops')
    workshop_warranty_statu = db.relationship('WorkshopWarrantyStatu', primaryjoin='Workshop.warranty_status == WorkshopWarrantyStatu.code', backref='workshops')
...
class WorkshopEquipmentDetail(Workshop):
    ...
    cf_text_workshop4 = db.Column(db.String, nullable=False, server_default=db.FetchedValue())
    cf_text_workshop5 = db.Column(db.String, nullable=False, server_default=db.FetchedValue())
    cf_text_workshop6 = db.Column(db.String, nullable=False, server_default=db.FetchedValue())
    cf_text_workshop7 = db.Column(db.String, nullable=False, server_default=db.FetchedValue())
    cf_text_workshop8 = db.Column(db.String, nullable=False, server_default=db.FetchedValue())
    cf_decimal_workshop1 = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())
    cf_decimal_workshop2 = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())
    cf_decimal_workshop3 = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())
    cf_bool_workshop1 = db.Column(db.Boolean, nullable=False, server_default=db.FetchedValue())
    cf_bool_workshop2 = db.Column(db.Boolean, nullable=False, server_default=db.FetchedValue())
    cf_bool_workshop3 = db.Column(db.Boolean, nullable=False, server_default=db.FetchedValue())
    cf_date_workshop1 = db.Column(db.Date, nullable=False, server_default=db.FetchedValue())
    cf_date_workshop2 = db.Column(db.Date, nullable=False, server_default=db.FetchedValue())

    workshop_equipment = db.relationship('WorkshopEquipment', primaryjoin='WorkshopEquipmentDetail.equipment_code == WorkshopEquipment.code', backref='workshop_equipment_details')
    workshop_equipment_statu = db.relationship('WorkshopEquipmentStatu', primaryjoin='WorkshopEquipmentDetail.equipment_status == WorkshopEquipmentStatu.code', backref='workshop_equipment_details')
...
class WorkshopAccessory(db.Model):
    ...
    workshop_accessories_type = db.relationship('WorkshopAccessoriesType', primaryjoin='WorkshopAccessory.accessories_type == WorkshopAccessoriesType.code', backref='workshop_accessories')
    workshop = db.relationship('Workshop', primaryjoin='WorkshopAccessory.main_correlative == Workshop.correlative', backref='workshop_accessories')
...
class WorkshopDocumentsRel(db.Model):
    ...
    workshop = db.relationship('Workshop', primaryjoin='WorkshopDocumentsRel.main_correlative == Workshop.correlative', backref='workshop_documents_rels')
...
class WorkshopEquipment(db.Model):
    ...
    cf_text_equipment4 = db.Column(db.String, nullable=False, server_default=db.FetchedValue())
    cf_text_equipment5 = db.Column(db.String, nullable=False, server_default=db.FetchedValue())
    cf_text_equipment6 = db.Column(db.String, nullable=False, server_default=db.FetchedValue())
    cf_text_equipment7 = db.Column(db.String, nullable=False, server_default=db.FetchedValue())
    cf_text_equipment8 = db.Column(db.String, nullable=False, server_default=db.FetchedValue())
    cf_decimal_equipment1 = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())
    cf_decimal_equipment2 = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())
    cf_decimal_equipment3 = db.Column(db.Double(53), nullable=False, server_default=db.FetchedValue())
    cf_bool_equipment1 = db.Column(db.Boolean, nullable=False, server_default=db.FetchedValue())
    cf_bool_equipment2 = db.Column(db.Boolean, nullable=False, server_default=db.FetchedValue())
    cf_bool_equipment3 = db.Column(db.Boolean, nullable=False, server_default=db.FetchedValue())
    cf_date_equipment1 = db.Column(db.Date, nullable=False, server_default=db.FetchedValue())
    cf_date_equipment2 = db.Column(db.Date, nullable=False, server_default=db.FetchedValue())

    workshop_equipment_type = db.relationship('WorkshopEquipmentType', primaryjoin='WorkshopEquipment.equipment_type == WorkshopEquipmentType.code', backref='workshop_equipments')
    workshop_mark = db.relationship('WorkshopMark', primaryjoin='WorkshopEquipment.mark == WorkshopMark.code', backref='workshop_equipments')
    workshop_model = db.relationship('WorkshopModel', primaryjoin='WorkshopEquipment.model == WorkshopModel.code', backref='workshop_equipments')
...
class WorkshopNotification(db.Model):
    ...
    workshop_notification_type = db.relationship('WorkshopNotificationType', primaryjoin='WorkshopNotification.notification_type == WorkshopNotificationType.code', backref='workshop_notifications')
    workshop_notification_statu = db.relationship('WorkshopNotificationStatu', primaryjoin='WorkshopNotification.status == WorkshopNotificationStatu.code', backref='workshop_notifications')
